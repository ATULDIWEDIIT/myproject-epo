...
//Global.AddPartialView = function ($url,$divId) {
//        $.ajax({
//            url: $url,
//            type: 'get',
//            success: function (result) {
//                $('#' + $divId).empty().html(result);
...
$(document).ready(function () {
    ...
    if ($('.youtube-video').length > 0) {
        ...
        $('.youtube-video').each(function (index, element) {
            var orgSrc = $(element).attr('data-src');
            var html = orgSrc.replace(/(?:https:\/\/)?(?:www\.)?(?:youtube\.com|youtu\.be)\/(?:watch\?v=)?(.+)/g, '<iframe width="685" height="313" src="https://www.youtube.com/embed/$1" frameborder="0" allowfullscreen></iframe>');
            $(element).html(html);
        });
